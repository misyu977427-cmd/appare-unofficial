...
type News = {
  id: string;
  title: string;
  publishedAt: string; // ✅ microCMSの標準フィールド名を使用
};
...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-900 text-black text-black">
      {/* HEADER */}
      <header className="py-6 px-5 border-b-8 border-black sticky top-0 bg-white z-50">
        <div className="max-w-6xl mx-auto flex justify-between items-center text-black">
          ...
          <nav className="hidden md:flex gap-6 font-black text-sm uppercase text-black">
            <Link href="/" className="hover:underline underline-offset-4 text-black text-black text-black">Home</Link>
            <Link href="#members" className="hover:underline underline-offset-4 text-black text-black text-black">Members</Link>
            <Link href="#news" className="hover:underline underline-offset-4 text-black text-black text-black">News</Link>
          </nav>
        </div>
      </header>
      ...
      <section className="bg-black text-white py-24 px-5 text-center">
        ...
        <p className="max-w-2xl mx-auto text-xl font-bold opacity-80 leading-relaxed text-white text-white">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-8">
            {newsData.contents.map((item) => (
              <Link href={`/news/${item.id}`} key={item.id} target="_blank" className="group block border-b-4 border-black pb-6 hover:bg-white transition-colors p-4 text-black">
                <p className="text-sm font-black text-gray-500 mb-2">
                  {/* ✅ Dateの修正：publishedAt を使用 */}

...
type News = { id: string; title: string; publishedAt: string; }; // ✅ date ではなく publishedAt を使用
...
export default async function Home() {
  const [memberData, newsData] = await Promise.all([
    ...
    client.getList<News>({ 
      endpoint: "news", 
      queries: { orders: '-publishedAt', limit: 3 } // 最新3件を取得
    }),
  ]);
            ...
            {newsData.contents.map((item) => (
              <Link href={`/news/${item.id}`} key={item.id} target="_blank" className="group block border-b-4 border-black pb-6 hover:bg-white transition-colors p-4 text-black">
                <p className="text-sm font-black text-gray-400 mb-2">
                  {/* ✅ Date の修正：publishedAt が無い場合は現在時刻で回避 */}
                  {new Date(item.publishedAt || new Date()).toLocaleDateString('ja-JP').replace(/\//g, '.')}
                </p>

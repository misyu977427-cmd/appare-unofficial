  ...
  return (
    <main style={{ backgroundColor: 'white', color: 'black', minHeight: '100vh', padding: '40px 20px', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: '3rem', fontWeight: '900', fontStyle: 'italic', borderBottom: '10px solid black', paddingBottom: '20px', marginBottom: '60px' }}>NEWS ARCHIVE</h1>
        <div style={{ display: 'grid', gap: '20px' }}>
          {data.contents.map((item: any) => (
            <Link href={`/news/${item.id}`} key={item.id} style={{ textDecoration: 'none', color: 'black' }}>
              <div style={{ borderBottom: '4px solid black', padding: '20px 0' }}>
                <p style={{ fontSize: '0.9rem', fontWeight: '900', color: '#6b7280' }}>
                  {new Date(item.publishedAt).toLocaleDateString('ja-JP').replace(/\//g, '.')}
                </p>
                <h3 style={{ fontSize: '1.5rem', fontWeight: '900', fontStyle: 'italic' }}>{item.title}</h3>
              </div>
            </Link>
          ))}
        </div>
